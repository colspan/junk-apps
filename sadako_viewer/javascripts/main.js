(function() {
	...
	function init() {
		...
		// 貞子
		var sadako = new THREE.PlaneGeometry(10, 10);
		var material_sadako = new THREE.MeshBasicMaterial({map:THREE.ImageUtils.loadTexture('./images/textures/sadako.jpg')});
		var mesh_sadako = new THREE.Mesh(sadako, material_sadako);
		mesh_sadako.position.x = -300;
		mesh_sadako.rotation.y = Math.PI/2;
		scene.add(mesh_sadako);
		window.setInterval(function(){mesh_sadako.position.x +=10;if(mesh_sadako.position.x>0) mesh_sadako.position.x=-300;},50);
